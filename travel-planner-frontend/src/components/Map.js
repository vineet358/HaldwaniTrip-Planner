...
export default function MapComponent() {
  ...
  const getRoute = async (sourceCoord, destCoord) => {
    try {
      // Use OSRM service to get route between coordinates
      const response = await axios.get(
        `https://router.project-osrm.org/route/v1/driving/${sourceCoord.lon},${sourceCoord.lat};${destCoord.lon},${destCoord.lat}?overview=full&geometries=geojson`
      );
